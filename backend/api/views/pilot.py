...
from core.business import (
    ...
    _clean_label, _unique_named_path, _IMAGE_EXTS, maybe_convert_heic,
...
def verify_image_content(uploaded):
    ...
    If Pillow is not installed, verification is skipped (returns True) so
    uploads are not blocked in environments without PIL."""
    try:
        from PIL import Image as _PILImage
    except ImportError:
        # Pillow not installed — skip content check, allow upload
        return True
    # NOTE: Django already rewinds the parsed upload to position 0
    # (MemoryFileUploadHandler/TemporaryFileUploadHandler call seek(0) in
    # file_complete()), so PIL would read fine even without an explicit seek.
    # We still seek(0) defensively in case an earlier handler consumed the
    # stream, and we operate on uploaded.file (the raw BytesIO/temp file) which
    # always supports seek/tell. The original Flask port crashed here because it
    # used `uploaded.stream`, which does not exist on a Django UploadedFile.
    try:
        raw = uploaded.file
        raw.seek(0)
        img = _PILImage.open(raw)
        img.verify()
        raw.seek(0)  # rewind so _save_django_file's chunks() reads from start
        return True
    except Exception as exc:
        # Don't fail silently — a swallowed error here previously looked like a
        # phantom "invalid image". Log the real reason, then reject the upload.
        try:
            import logging
            logging.getLogger(__name__).warning(
                '[upload-render] verify_image_content rejected %r: %s',
...
def pilot_job(request, order_id):
    ...
    # ── Instagram Carousel: gather URLs + parse saved plan into slide cards ──
    carousel_urls, carousel_plan_obj = [], None
    if o['task'] == 'instagram_carousel':
        carousel_urls = intake.get('brand_urls') or (
            [intake['brand_url']] if intake.get('brand_url') else [])
        raw_plan = order.get('carousel_plan')
        if raw_plan:
            try:
                carousel_plan_obj = json.loads(raw_plan)
            except Exception:
                carousel_plan_obj = None
    ...
    ctx = {
        ...
        'carousel_plan_obj': carousel_plan_obj,
...
# ── /api/pilot/save-brand-dna (Instagram Carousel — Stage 1) ─
@require_POST
@role_required('pilot', 'admin')
def api_save_brand_dna(request):
    data = body(request)
    order_id = data.get('order_id')
    brand_dna = data.get('brand_dna', '').strip()
    conn = get_db()
    o = conn.execute('SELECT * FROM orders WHERE id = ? AND assigned_pilot_id = ?',
                     (order_id, request.user.id)).fetchone()
    if not o:
        conn.close()
        return err('Not found', status=404)
    conn.execute("UPDATE orders SET brand_dna=? WHERE id=?", (brand_dna, order_id))
    conn.commit(); conn.close()
    return ok({'success': True})


# ── /api/pilot/save-carousel-plan (Instagram Carousel — Stage 2) ─
@require_POST
@role_required('pilot', 'admin')
def api_save_carousel_plan(request):
    data = body(request)
    order_id = data.get('order_id')
    plan = data.get('carousel_plan', '').strip()
    conn = get_db()
    o = conn.execute('SELECT * FROM orders WHERE id = ? AND assigned_pilot_id = ?',
                     (order_id, request.user.id)).fetchone()
    if not o:
        conn.close()
        return err('Not found', status=404)
    conn.execute("UPDATE orders SET carousel_plan=? WHERE id=?", (plan, order_id))
    conn.commit(); conn.close()
    return ok({'success': True})
