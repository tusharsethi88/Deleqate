...
def api_pilot_qc(request):
    ...
    if all_passed and len([s for s in all_steps if s['render_filename']]) == len(all_steps):
        ...
        conn.execute("UPDATE orders SET status='under_review' WHERE id=?", (order_id,))
        log_status(conn, order_id, o['status'], 'under_review', current_user.id, 'All POVs passed QC')
...
def pilot_submit(request, order_id):
    ...
    if o and o['status'] in ('assigned', 'in_progress', 'rejected', 'edit_requested'):
        if request.POST.get('qc_confirmed') != '1':
            conn.close()
            # Tell the pilot why submit was blocked (template reads ?blocked=).
            return HttpResponseRedirect(f'/pilot/job/{order_id}?blocked=qc')
        deliverable_count = conn.execute(
            "SELECT COUNT(*) FROM deliverables WHERE order_id=?", (order_id,)).fetchone()[0]
        if deliverable_count == 0:
            conn.close()
            return HttpResponseRedirect(f'/pilot/job/{order_id}?blocked=no_deliverable')
        conn.execute("UPDATE orders SET status='under_review', client_action=NULL WHERE id=?", (order_id,))
...
def api_delete_deliverable(request):
    ...
    o = conn.execute('SELECT status FROM orders WHERE id=?', (order_id,)).fetchone()
    if o and o['status'] in ('submitted', 'under_review', 'delivered', 'approved'):
        conn.close()
...
@require_POST
@role_required('pilot', 'admin')
def api_pilot_unassign(request):
    """Return a task to the unassigned pool. Only allowed while status='assigned'
    (i.e. before the pilot has begun executing / uploading anything)."""
    current_user = request.user
    data = body(request)
    order_id = data.get('order_id')
    if not order_id:
        return err('Missing order_id', success=False)
    conn = get_db()
    if current_user.role == 'admin':
        o = conn.execute('SELECT * FROM orders WHERE id = ?', (order_id,)).fetchone()
    else:
    ...
    if o['status'] != 'assigned':
        conn.close()
        return err('Task can only be returned before you begin executing it.', success=False)
    conn.execute(
        "UPDATE orders SET assigned_pilot_id=NULL, status='pending', assigned_at=NULL WHERE id=?",
        (order_id,))
    log_status(conn, order_id, 'assigned', 'pending', current_user.id, 'Pilot returned task to pool')
    conn.commit(); conn.close()
    return ok({'success': True})
