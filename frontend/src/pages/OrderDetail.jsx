...
.od-logo{display:block;}
.od-logo img, .od-logo video{height:78px;width:auto;display:block;mix-blend-mode:multiply;}
        ...
        <div className="od-card">
                ...
                {canQC && (
                  <>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className={`btn-approve-img ${st === 'approved' ? 'btn-disabled' : ''}`} onClick={() => reviewImg(dv.id, 'approve')} style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                        <i data-lucide="check-circle" style={{ width: 14, height: 14 }}></i> Approve
                      </button>
                      <button className={`btn-reject-img ${st === 'rejected' ? 'btn-disabled' : ''}`} onClick={() => reviewImg(dv.id, 'reject')} style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                        <i data-lucide="x-circle" style={{ width: 14, height: 14 }}></i> Reject
                      </button>
                    </div>
                  </>
                )}
          ...
          {canQC && (
            <div style={{ marginTop: '1rem', borderTop: '1px solid #DDE3EC', paddingTop: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn-approve-img" onClick={() => orderQC('approve')} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                  <i data-lucide="check" style={{ width: 15, height: 15 }}></i> QC Pass — Deliver to Client
                </button>
                <button className="btn-reject-img" onClick={() => orderQC('reject')} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                  <i data-lucide="x" style={{ width: 15, height: 15 }}></i> Reject All — Back to Pilot
                </button>
              </div>
            </div>
          )}
          ...
          {isAdmin && order.status === 'delivered' && order.payment_method && order.payment_method !== 'payu' && order.payment_method !== 'free_pass' && (
            <button className="btn-approve-img" style={{ marginTop: '1rem', display: 'inline-flex', alignItems: 'center', gap: '6px' }} onClick={confirmPayment}>
              <i data-lucide="check" style={{ width: 15, height: 15 }}></i> Confirm Payment &amp; Unlock Download
            </button>
          )}
        </div>
      </div>
