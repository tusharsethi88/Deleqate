...
.cp-top .brand{font-weight:800;color:#fff;font-size:1.1rem;text-decoration:none;}
.cp-top .brand span{color:var(--gold);}
  ...
  return (
    <div>
      ...
      <div className="cp-top">
        <Link to="/" className="brand">Dele<span>qate</span></Link>
        <span className="cp-top-order">Order #{order.id} · {d.task_label}</span>
        <Link to="/client/orders" className="back-link">← Back to My Orders</Link>
      </div>
      ...
      <div className="cp-wrap">
            ...
            <div className="cp-deliv" key={dv.id}>
              ...
              {showSlider
                ...
                : isImg(dv.filename)
                  ...
                  : <div style={{ padding: '1.5rem', textAlign: 'center' }}><a href={fileUrl(`/deliverables/${dv.filename}`)} style={{ color: '#C9A84C', fontWeight: 700 }} target="_blank" rel="noreferrer">↓ {dv.original_name || dv.filename}</a></div>}
            </div>
          );
        })}
        ...
        {order.status === 'approved' && (
          <div className="action-wrap"><div className="action-card" style={{ borderColor: '#16A34A' }}>
            <h3 style={{ color: '#16A34A', margin: 0 }}>✅ Approved</h3>
            <p>You've approved this delivery. <Link to={`/order/success?id=${order.id}`}>Download your files →</Link></p>
          </div></div>
        )}
        ...
        {order.status === 'delivered' && !d.has_paid_before && (
          <div className="action-wrap">
            <div className="tabs">
              <button className={`tab-btn ${tab === 'pay' ? 'active' : ''}`} onClick={() => setTab('pay')}>✅ Pay &amp; Download</button>
              <button className={`tab-btn ${tab === 'edit' ? 'active' : ''}`} onClick={() => setTab('edit')}>✏️ Pay with Edit</button>
            ...
            {tab === 'pay' && (
              <div className="action-card" style={{ borderColor: '#2E6B2E' }}>
                ...
                <button className="btn-action-primary" disabled={busy} onClick={() => submitAction('pay_download', false)}>💳 Pay {totalRupees} &amp; Download</button>
              </div>
            )}
            {tab === 'edit' && (
              <div className="action-card" style={{ borderColor: '#C9A84C' }}>
                ...
                <button className="btn-action-primary btn-gold" disabled={busy} onClick={() => submitAction('pay_edit', true)}>✏️ Submit Edits &amp; Pay {totalRupees}</button>
              </div>
            )}
            {tab === 'reject' && (
              <div className="action-card" style={{ borderColor: '#7F1D1D' }}>
                ...
                <button className="btn-action-danger" disabled={busy} onClick={() => { if (!reject.trim()) { flash('Please enter a reason.', false); return; } if (window.confirm('Submit rejection?')) submitAction('reject', false); }}>❌ Submit Rejection</button>
              </div>
            )}
          </div>
        )}
        ...
        {order.status === 'delivered' && d.has_paid_before && (
          <div className="action-wrap">
            <div className="tabs">
              <button className={`tab-btn ${tab === 'accept' ? 'active' : ''}`} onClick={() => setTab('accept')}>✅ Accept &amp; Download</button>
              <button className={`tab-btn ${tab === 'cedit' ? 'active' : ''}`} onClick={() => setTab('cedit')}>✏️ Request Edit{credits > 0 ? ` (${credits} free)` : ''}</button>
            </div>
            {tab === 'accept' && (
              <div className="action-card" style={{ borderColor: '#2E6B2E' }}>
                <p>Your payment is already on file — accept to unlock your final files.</p>
                <button className="btn-action-primary" disabled={busy} onClick={() => submitAction('accept', false)}>✅ Accept &amp; Download</button>
              </div>
            )}
            {tab === 'cedit' && (
              <div className="action-card" style={{ borderColor: '#C9A84C' }}>
                ...
                <button className="btn-action-primary btn-gold" disabled={busy} onClick={() => submitAction('edit_free', true)}>✏️ Submit Edit Request</button>
              </div>
            )}
          </div>
        )}
