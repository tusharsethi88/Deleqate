...
function summary(o) {
  ...
  switch (o.task) {
    ...
    case 'instagram_carousel': return `${i.carousel_topic || '—'} · ${i.num_slides || '7'} slides`;
...
function pillLabel(s) {
  const map = {
    pending: { icon: 'clock', text: 'With Admin' },
    assigned: { icon: 'user', text: 'Assigned' },
    in_progress: { icon: 'refresh-cw', text: 'In Progress' },
    under_review: { icon: 'search', text: 'Under Review' },
    delivered: { icon: 'check-circle', text: 'Delivered' },
    approved: { icon: 'check-circle', text: 'Delivered' }
  };
  const item = map[s] || { icon: 'info', text: s.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) };
  return (
    <>
      <i data-lucide={item.icon} style={{ width: 13, height: 13, marginRight: 5 }}></i>
      <span>{item.text}</span>
    </>
  );
}
    ...
    <div>
      ...
      <nav className="navbar">
        ...
        <div className="navbar-nav" style={navOpen ? { display: 'flex' } : undefined}>
          ...
          {userName && <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--gray-600)' }}>{userName}</span>}
        ...
        <div className="page-header">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
            <span title="Edit credits let you request revisions on delivered orders"
              ...
              <i data-lucide="sparkles" style={{ width: 13, height: 13 }}></i> {credits} edit credit{credits !== 1 ? 's' : ''}
            </span>
        ...
        {orders.length ? (
                ...
                <div key={o.id}
                  ...
                  <div className="order-actions">
                    ...
                    {(cur === 'delivered' || cur === 'approved') && (
                      <Link to={`/order/${o.id}/preview`} className="btn-review" onClick={e => e.stopPropagation()} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                        {cur === 'approved' ? (
                          <>
                          ...
                          <>
                            <i data-lucide="eye" style={{ width: 14, height: 14 }}></i>
                            Review &amp; Approve
                          </>
                        )}
                      </Link>
          ...
          <div className="empty-state">
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}>
              <i data-lucide="clipboard-list" style={{ width: 56, height: 56, color: 'var(--gray-300)' }}></i>
            </div>
            ...
            <Link to="/order" className="btn btn-gold btn-lg" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>Place First Order <i data-lucide="arrow-right" style={{ width: 16, height: 16 }}></i></Link>
          </div>
        )}
      </div>
    </div>
