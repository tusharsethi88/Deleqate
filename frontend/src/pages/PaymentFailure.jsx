...
export default function PaymentFailure() {
  ...
  const supportWa = params.get('support_whatsapp') || '917011989292';
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: 'var(--surface-low)', padding: '1.5rem' }}>
      <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-lg)', padding: '3rem 2.5rem', maxWidth: 460, width: '100%', textAlign: 'center' }}>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}>
          <i data-lucide="x-circle" style={{ width: 56, height: 56, color: '#dc2626' }}></i>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <a href={`https://wa.me/${supportWa}?text=Hi%2C+I+had+a+payment+issue+for+Order+%23${orderId || ''}`}
            className="btn" style={{ background: 'var(--cream)', color: 'var(--navy)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }} target="_blank" rel="noreferrer">
            <i data-lucide="message-square" style={{ width: 14, height: 14 }}></i> WhatsApp Support
          </a>
        </div>
      </div>
    </div>
