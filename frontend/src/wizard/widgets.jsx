        ...
        <div className="multi-file-list">
          {names.map((n, i) => (
            <span key={i} className="file-chip-item">
              <span className="fc-name">📎 {n.length > 22 ? n.slice(0, 20) + '…' : n}</span>
            </span>
          ))}
        </div>
      ) : (
        names[0] && <div className="file-shown" style={compact ? { fontSize: '0.68rem' } : undefined}>✓ {names[0]}</div>
      )}
...
export function PriceCard({ amount, breakdown, sla }) {
  return (
    <div className="price-card">
      ...
      <div className="price-sla">⏱ {sla}</div>
    </div>
  );
}
